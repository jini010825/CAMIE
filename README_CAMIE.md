...
CAMIE is a post-hoc explanation framework for **multi-task molecular GNNs**.  
Unlike context-agnostic explainers that assign a single fixed explanation to a molecule, CAMIE estimates **assay-specific motif importance** for each **molecule–assay pair**.

The key idea is that, in multi-task molecular prediction, the same molecule may rely on different substructures depending on the queried biological assay. CAMIE captures this by conditioning motif scoring on assay context and distilling assay-specific motif-removal responses from a frozen multi-task predictor.
...
2. **Motif decomposition and representation extraction**  
   Each molecule is decomposed into chemically meaningful candidate motifs using BRICS.  
   Graph, motif, and assay-context representations are then constructed.
...
> Replace the figure below with the paper figure used in the manuscript.

```text
[Insert CAMIE overview figure from the paper here]
```
...
to estimate this assay-specific motif-removal response.

The final explanation is obtained by ranking motifs according to their predicted scores and selecting the top-k motifs.
...
.
├── assets/
│   ├── assay_emb/                 # assay-context embeddings
│   ├── baselines/                 # baseline outputs and summaries
│   ├── motif_split/               # motif decomposition mappings
│   ├── scoring/                   # scorer outputs, decompositions, summaries
│   └── toxcast_gnn/               # pretrained GNN checkpoints and embeddings
│
├── baselines/
│   ├── common/                    # shared utilities for baselines
│   ├── evaluate/                  # fidelity and evaluation scripts
│   └── models/
│       ├── exel/                  # eXEL-based baselines
│       ├── gradient/              # SA, GBP, GradCAM
│       └── perturbation/          # GNNExplainer, PGExplainer
│
├── datasets/
│   ├── processed/                 # processed ToxCast tables
│   └── raw_data/                  # raw molecular assay data
│
├── models/
│   └── gnn/                       # multi-task molecular GNN backbone
│
├── subgraph/
│   └── scoring/
│       ├── train_mse.py           # CAMIE scorer training
│       ├── scores_mse.py          # motif scoring with trained scorer
│       └── decomposition_mse.py   # top-k motif decomposition
│
├── analysis/                      # additional analysis scripts and outputs
├── README.md
└── environment.yml
```
...
For a molecule–assay pair, CAMIE selects the top-k motifs as the explanation set \(S_2\).  
The remaining motifs are denoted by \(S_1\).
...
```bibtex
@inproceedings{camie2026,
  title={Context-Aware Motif Importance Explanations for Multi-Task GNNs},
